...
public class HbMemberTest extends JFrame implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(jb == jb1){
		 jta.setText("");
		   HBDAO dao = new HBDAO();
		   ArrayList<HBVO> hvo = dao.getSelect();
		   jta.append("회원 전체정보\n");
		   ...
		   for(HBVO k : hvo){
			...
			jta.setText("");
			HBDAO dao = new HBDAO();
			   HBVO vo = dao.getSelectOne(jtf1.getText());
			   jta.append(jtf1.getText()+"님의 개인정보\n");
			   jta.append("  idx\tid\tpwd\tname\tage\taddr\n");
			  
				   jta.append("  "+vo.getIdx()+"\t");
				   jta.append(vo.getId()+"\t");
				   jta.append(vo.getPwd()+"\t");
				   jta.append(vo.getName()+"\t");
				   jta.append(vo.getAge()+"\t");
				   jta.append(vo.getAddr()+"\n");
		}else if(jb == jb2){
			HBDAO dao = new HBDAO();
